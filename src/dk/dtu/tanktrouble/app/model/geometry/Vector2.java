...
public record Vector2(double x, double y) implements Serializable {
	...
	public Vector2 moveAndRotate(Vector2 p, double angle) {
		return new Vector2(p.x() + x * Math.cos(angle) - y * Math.sin(angle), p.y() + x * Math.sin(angle) + y * Math.cos(angle));
	}
	...
	public static Vector2[] generateVectors(double[] x, double[] y) {
		if (x.length != y.length) return null;

		Vector2[] pts = new Vector2[x.length];
